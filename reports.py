...
class ReportsWidget(QWidget):
    ...
    def plot_sales_trends(self, sales):
    ...
    def populate_top_products(self, sales):
        product_totals = {}
        product_quantities = {}
    ...
    def populate_top_customers(self, sales):
        cust_totals = {}
        cust_bills = {}
    ...
    def show_low_stock_alerts(self):
        ...
        if not os.path.exists(path):
            self.low_stock_label.setText("Purchase data file not found for low stock alerts.")
            return
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                ...
                qty = row[4]
                expiry_str = row[8]
                if pname not in product_quantities:
                    product_quantities[pname] = 0.0
                    product_expiries[pname] = None
                product_quantities[pname] += qty
                if expiry_str:
                    try:
                        expiry_date = datetime.strptime(expiry_str, "%d-%m-%Y")
                        if (product_expiries[pname] is None) or (expiry_date < product_expiries[pname]):
                            product_expiries[pname] = expiry_date
                    except Exception:
                        pass

            low_stock_items = [p for p, q in product_quantities.items() if q <= 5]  # threshold for low stock
            near_expiry_items = [p for p, dt in product_expiries.items() if dt and 0 <= (dt - now).days <= 30]
            ...
            self.low_stock_label.setText(f"Error loading stock data: {str(e)}")
